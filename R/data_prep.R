...
x <- all_prof |> 
  ...
  right_join(make_mke_rc(private_type = "all") |> 
  ...
  select(school_year, 
         ...
         starts_with("per_"),
         report_card_type) |> 
  mutate(accurate_agency_type = case_when(dpi_true_id %in% city_charters ~ "Public Charter School",
                                          accurate_agency_type %in% mps ~ "Milwaukee Public School",
