...
all_prof <- all_prof_tmp |> 
  ...
  # below filter removes schools where no_test is 100%
...
xx <- all_prof |> 
  pivot_wider(names_from = test_subject, values_from = pa) |> 
  select(-c(accurate_agency_type, school_year)) |> 
  right_join(make_wi_rc(private_type = "all", exclude_milwaukee = FALSE) |> 
               filter(school_year == sy2) |> 
               select(dpi_true_id,
                      school_year,
                      school_name,
                      accurate_agency_type,
...
saveRDS(xx |> 
        ...
        "data/milwaukee_schools_2025.rda")

# not sure we want to share this with public
...
x <- left_join(xx, cfc_scored) |> 
  rename(dpi_rating = overall_rating,
         dpi_score = overall_score,
